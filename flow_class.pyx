...
cdef double M_PI = cmath.M_PI
cdef double SQRT2 = 2.**0.5
cdef double D2R = 180./M_PI
...
cdef class Cell:
    ...
    cdef int rowindex, colindex 
    cdef double altitude, cellsize, flux, z_delta, alpha, exp, flux_threshold, max_z_delta
    
    def __init__(self, int rowindex, int colindex, dem_ng, double cellsize, double flux, double z_delta,
    parent, double alpha, double exp, double flux_threshold, double max_z_delta, startcell):
    ...
    def calc_tanbeta(self):
        ds = np.array([[SQRT2, 1, SQRT2], [1, 1, 1], [SQRT2, 1, SQRT2]])
